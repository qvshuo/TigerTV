...
| 命令 | 参数 | 说明 |
|---|---|---|
| `search` | `<keyword>` | 搜索关键字，单站最多取 100 条，输出 JSON |
| `fetch` | `--site <site> --vod_id <id>` | 精准匹配站点并获取播放/下载链接，输出 JSON |
...
- 日志统一写入 `/tmp/tiger-tv.log`，格式为 `时间戳 [级别] [站点]: 消息`，所有上下文统一使用站点名。

## 站点名称说明

`fetch --site` 需使用配置中的完整 `name`，例如：
...
```bash
tiger-tv.py fetch --site test --vod_id 1
```
...
**Q: 搜索结果为 0？**  
A: 先检查网络是否可访问 GitHub RAW 和目标资源站 API；部分站点可能临时失效。

**Q: `fetch` 提示“未找到站点”？**  
A: `--site` 必须完整匹配配置中的 `name` 字段，包括其中的 emoji 或分隔符。
